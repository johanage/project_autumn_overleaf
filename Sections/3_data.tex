...
\subsection{Experimental database Juice}

Experimental data from IPP's Juice database has been used in this paper for the radiated power scaling. It is a database of global plasma parameters throughout the first operational campaign of W7-X where every observable have been averaged over a time interval of $200 \mathrm{ ms}$ for denoising purposes. The observables in the database comes from the W7X archive which is a web based service for accessing data measured by the diagnostics.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        $p_{neutral}$ & $\mathrm{Pa}$ & $\mathrm{mBar}$ \\\hline
...
The line-averaged effective ion charge number is inferred from bremsstrahlung spectrometer data using a Bayesian model based on the Minerva framework which infers profiles with Gaussian processes [\cite{pavone2019measurements}]. The equation which is solved to obtain the line-averaged effective ion charge is given by:
\begin{equation}
\label{eq:minerva_zeff}
    S(\lambda) = C(\lambda)V(\lambda) = C(\lambda) \int g_{ff}(Z_{eff}, T_e, \lambda) \frac{n_e^2 Z_{eff}}{\sqrt{k_b T_e}\exp{\left( \frac{hc}{\lambda k_b T_e}\frac{1}{\lambda^2}dl \right)}}
\end{equation}

where the expected measured signal $S(\lambda)$ can be calculated from the bremsstrahlung emission $V(\lambda)$ at a given wavelength $\lambda$. The electron temperature $T_e$ and density $n_e$ is gathered from Thomson Scattering data. The line of sight path $g_{ff}(Z_{eff}, T_e, \lambda)$ is the Gaunt factor measure in the Minerva system with $C(\lambda)$ as a calibration factor. To solve for the line-averaged value of $Z_{eff}$ it is assumed constant and the expected measured signal is divided by the integral on the right hand side of Eq. \ref{eq:minerva_zeff}.
...
The intensity of the emission from impurity and neutral radiation is measure by the Coherence Imaging Spectroscopy (CIS). It is a camera-based interferometric system which measures the Doppler particle flow velocities and temperature associated with a selection of visible emission lines form the plasma (\cite{perseo2020coherence}). The interference pattern is generated by birefringent crystals and overlaid on camera images of the plasma visible radiation, allowing 2D LOS integrated measurements. The choice of impurities and neutral is selected by narrowband filters. It is also possible to select single charged state of single particle species which depend on the installed filter.
