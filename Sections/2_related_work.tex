...
Valeria Perseo's scaling with $I_{C^{2+}}$
...
Having energy balance limits the amount and type of impurity that can be present in the plasma. The presence of impurities leads to increase radiation and cooling of the discharge by bremsstrahlung in the core, line radiation and recombination radiation according to \cite{rebut1977effect}. High $Z$-materials will typically not be fully ionized, but radiates strongly and vice versa for low-$Z$ materials. The trade-off is the level of radiation against the level if ionization and the tolerance of contamination. For high-$Z$ materials the level of contamination is lower than for low-$Z$ materials.
...
However interesting, this model is suitable for machines where the magnetic geometry allows for a reasonable calculation of the radiation surface area $S$ which in reality is hard to compute, especially for stellarators which have complex magnetic geometries. A crude assumption often made is that the radiation surface area covers the area of the sum of the divertor plates. In reality this is not the case as demonstrated by \cite{effenberg2019first}.
